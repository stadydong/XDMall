		...
		<div class=" cart-info" v-if="carList.length != 0">
					...
					<tr v-for="(item, index) in carList" :key="item.id" class="border">
						<td class="flex items-center pl-4">
							<input
								...
								@change="changeChecked(item.checked, item.id)"
							/>
						...
						<td>
							<button @click="checkout(item)">删除</button>
						</td>
					</tr>
				</tbody>
			</table>
			<div class="flex justify-between items-center py-4 border border-t-0">
				...
				<div class="right flex items-center text-base">
					<div>
						已选择 <span class="text-xl text-red-600">{{ selectCount }}</span> 件商品
					</div>
					<div class="px-4 text-zinc-300">|</div>
					<div class="">
						应付总额：<span class="text-xl text-red-600">￥{{ selectProductPrice }}</span>
					</div>
					<button class="bg-blue-600 text-white px-6 py-3 rounded-lg mx-4" @click="payment">现在结算</button>
				</div>
			</div>
		</div>
    <div class="py-4 flex items-center flex-col border border-t-0" v-else>
      <img src="@/../public/img/cart.png" class=" w-72">
      <div class=" my-3">你的购物车空空如也</div>
      <router-link to="/goods">
        <button class=" px-4 py-2 bg-zinc-100 rounded border text-zinc-500">前往选购</button>
      </router-link>
    </div>
		<!-- 全局对话框结算时打开 -->
...
import { changeCarChecked, getCodeImg, Payfail,deleteCarShopping } from '@/service/cart/cart'
...
export default {
	...
	methods: {
		...
		/**删除当前商品 */
		async checkout(carshoppign) {
      const deleteResult = await deleteCarShopping([carshoppign.id])
      if(deleteResult.success){
        this.$message.success(deleteResult.data.message)
			  this.$store.dispatch('GetCarList')

      }else{
        ErrorMessageAlert(deleteResult.data,this)
      }
    },
		/**结账 */
		async payment() {
